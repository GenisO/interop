...
csv_size = 6        # NULL or num
csv_user_type = 7   # string
        ...
        if is_folder:
            if user_id in server_folder_dict:
                if node_id in node_server_id_dict:
                    ...
                    raise ValueError("Error user %d does not have any folder to delete" %(user_id))
            else:
                raise ValueError("Error user %d does not uploaded any folder" %(user_id))
        else:
            if user_id in server_file_dict:
                if node_id in node_server_id_dict:
                    server_id = node_server_id_dict[node_id]
        ...
        if is_folder:
            if user_id in server_folder_dict:
                if node_id in node_server_id_dict:
                    ...
                    raise ValueError("Error user %d does not have any folder to move" %(user_id))
            else:
                raise ValueError("Error user %d does not uploaded any folder" %(user_id))
        else:
            if user_id in server_file_dict:
                if node_id in node_server_id_dict:
                    ...
                    raise ValueError("Error user %d does not have any file to move" %(user_id))
            else:
                raise ValueError("Error user %d does not uploaded any file" %(user_id))
